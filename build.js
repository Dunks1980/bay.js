...
fs.readdir(path.join(__dirname, 'src'), (error, files) => {
  ...
  files.forEach( file => {
    ...
    const outputFile = `(function(){${bay}var ea=[];var e=document.querySelector('${filename}');if (e) {ea = Array.from(e.attributes).map(a => a.name);}bay.create('${filename}',\`${template}\`,ea);}())`;
